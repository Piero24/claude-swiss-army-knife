...
def create_server() -> Server:
    ...
    async def list_tools() -> list[Tool]:
        return [
            Tool(name="obsidian_list_vault", description="List the vault directory structure.",
                 inputSchema={"type": "object", "properties": {
                     "subfolder": {"type": "string", "description": "Subfolder to list (default: root)."},
                     "depth": {"type": "integer", "description": "Max depth (default: 3)."},
                 }}),
            Tool(name="obsidian_read_note", description="Read a note's full content with parsed frontmatter.",
                 inputSchema={"type": "object", "properties": {
                     "path": {"type": "string", "description": "Path relative to vault root."},
                 }, "required": ["path"]}),
            Tool(name="obsidian_write_note", description="Create or update a note.",
                 inputSchema={"type": "object", "properties": {
                     "path": {"type": "string", "description": "Path relative to vault root."},
                     "content": {"type": "string", "description": "Markdown content."},
                     "frontmatter": {"type": "object", "description": "Optional YAML frontmatter to merge."},
                 }, "required": ["path", "content"]}),
            Tool(name="obsidian_delete_note", description="Delete a note (soft-delete to .trash/ by default).",
                 inputSchema={"type": "object", "properties": {
                     "path": {"type": "string", "description": "Path relative to vault root."},
                     "permanent": {"type": "boolean", "description": "Permanently delete (default: false)."},
                 }, "required": ["path"]}),
            Tool(name="obsidian_search_notes", description="Full-text search across all notes using ripgrep.",
                 inputSchema={"type": "object", "properties": {
            ...
            Tool(name="obsidian_search_by_tag", description="Find all notes with a specific frontmatter tag.",
                 inputSchema={"type": "object", "properties": {
                     "tag": {"type": "string", "description": "Tag to search for."},
                 }, "required": ["tag"]}),
            Tool(name="obsidian_get_backlinks", description="Find notes that link to a target note via [[wikilinks]].",
                 inputSchema={"type": "object", "properties": {
                     "path": {"type": "string", "description": "Target note path."},
                 }, "required": ["path"]}),
            Tool(name="obsidian_get_tags", description="List all unique tags used across the vault.",
                 inputSchema={"type": "object", "properties": {}}),
            Tool(name="obsidian_get_frontmatter", description="Read only the YAML frontmatter of a note.",
                 inputSchema={"type": "object", "properties": {
                     "path": {"type": "string", "description": "Path relative to vault root."},
                 }, "required": ["path"]}),
        ]
        ...
        try:
            match name:
                case "obsidian_list_vault":
                    ...
                    entries = vault.list_vault(subfolder, arguments.get("depth", 3))
                    return [TextContent(type="text", text=json.dumps({"entries": entries, "count": len(entries)}, indent=2))]
                ...
                case "obsidian_delete_note":
                    enforcer.check("write", arguments["path"])
                    result = vault.delete_note(arguments["path"], arguments.get("permanent", False))
                ...
                case "obsidian_search_notes":
                    ...
                    results = _ripgrep_search(str(vault.root), query, max_results, regex)
                    return [TextContent(type="text", text=json.dumps({"results": results, "count": len(results)}, indent=2, ensure_ascii=False))]

                case "obsidian_search_by_tag":
                    enforcer.check("read", "/")
                    tag = arguments["tag"]
                    results = _search_by_tag(vault, tag)
                    return [TextContent(type="text", text=json.dumps({"tag": tag, "results": results, "count": len(results)}, indent=2))]

                case "obsidian_get_backlinks":
                    enforcer.check("read", arguments["path"])
                    backlinks = find_backlinks(vault.root, arguments["path"])
                    return [TextContent(type="text", text=json.dumps({"target": arguments["path"], "backlinks": backlinks, "count": len(backlinks)}, indent=2))]

                case "obsidian_get_tags":
                    enforcer.check("read", "/")
                    all_tags = _get_all_tags(vault)
                    return [TextContent(type="text", text=json.dumps({"tags": all_tags}, indent=2))]

                case "obsidian_get_frontmatter":
                    enforcer.check("read", arguments["path"])
                    content = vault.read_note(arguments["path"])
                    fm, _ = parse_frontmatter(content)
                    return [TextContent(type="text", text=json.dumps({"path": arguments["path"], "frontmatter": fm}, indent=2))]
...
def _ripgrep_search(vault_root: str, query: str, max_results: int = 20, regex: bool = False) -> list[dict]:
    """Search vault with ripgrep."""
    cmd = ["rg", "--type", "md", "--line-number", "--max-count", str(max_results)]
    if not regex:
    ...
    try:
        ...
        return [
            {"file": parts[0], "line": int(parts[1]), "snippet": ":".join(parts[2:]) if len(parts) > 2 else ""}
            for line in lines
...
async def main() -> None:
    ...
    logger.info("Loaded config — %d path rules", len(_enforcer.config.permissions.paths))

    _vault = Vault(VAULT_PATH)
    logger.info("Vault opened at %s (%d notes)", VAULT_PATH, len(_vault.get_all_notes()))
