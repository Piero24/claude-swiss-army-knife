...
def create_server() -> Server:
    ...
    async def list_tools() -> list[Tool]:
        return [
            Tool(name="syno_file_list", description="List files in a Synology shared folder.",
                 inputSchema={"type": "object", "properties": {
                     "folder_path": {"type": "string", "description": "Path within a shared folder."},
                     "limit": {"type": "integer", "description": "Max entries (default: 500)."},
                 }, "required": ["folder_path"]}),
            Tool(name="syno_file_read", description="Read a file from the Synology NAS.",
                 inputSchema={"type": "object", "properties": {
                     "file_path": {"type": "string", "description": "Full path to the file."},
                 }, "required": ["file_path"]}),
            Tool(name="syno_file_write", description="Write/upload a file to the Synology NAS.",
                 inputSchema={"type": "object", "properties": {
                     "folder_path": {"type": "string", "description": "Parent folder path."},
                     "filename": {"type": "string", "description": "Name of the file to create."},
                     "content": {"type": "string", "description": "File content."},
                 }, "required": ["folder_path", "filename", "content"]}),
            Tool(name="syno_file_delete", description="Delete a file or folder on the Synology NAS.",
                 inputSchema={"type": "object", "properties": {
                     "file_path": {"type": "string", "description": "Full path to delete."},
                     "recursive": {"type": "boolean", "description": "Recursively delete folders (default: false)."},
                 }, "required": ["file_path"]}),
            Tool(name="syno_file_move", description="Move/rename a file or folder on the Synology NAS.",
                 inputSchema={"type": "object", "properties": {
                     "src_path": {"type": "string", "description": "Source path."},
                     "dst_path": {"type": "string", "description": "Destination path."},
                 }, "required": ["src_path", "dst_path"]}),
            Tool(name="syno_file_search", description="Search for files by name on the Synology NAS.",
                 inputSchema={"type": "object", "properties": {
                     "query": {"type": "string", "description": "Search query (name pattern)."},
                     "folder_path": {"type": "string", "description": "Folder to search within (default: /)."},
                 }, "required": ["query"]}),
            Tool(name="syno_system_info", description="Get Synology NAS system info: model, DSM version, CPU, RAM.",
                 inputSchema={"type": "object", "properties": {}}),
            Tool(name="syno_storage_info", description="Get Synology NAS storage: volumes, usage, disk health.",
                 inputSchema={"type": "object", "properties": {}}),
        ]
        ...
        try:
            match name:
                case "syno_file_list":
                    ...
                    result = await dsm.file_list(arguments["folder_path"], arguments.get("limit", 500))
                    return [TextContent(type="text", text=json.dumps({"files": result, "count": len(result)}, indent=2))]

                case "syno_file_read":
                    enforcer.check("read", arguments["file_path"])
                    content = await dsm.file_read(arguments["file_path"])
                    return [TextContent(type="text", text=json.dumps({"content": content, "path": arguments["file_path"]}, indent=2))]

                case "syno_file_write":
                    enforcer.check("write", arguments["folder_path"])
                    result = await dsm.file_write(arguments["folder_path"], arguments["filename"], arguments["content"])
                    return [TextContent(type="text", text=json.dumps(result, indent=2))]

                case "syno_file_delete":
                    enforcer.check("write", arguments["file_path"])
                    result = await dsm.file_delete(arguments["file_path"], arguments.get("recursive", False))
                    return [TextContent(type="text", text=json.dumps(result, indent=2))]

                case "syno_file_move":
                    enforcer.check("write", arguments["src_path"])
                    enforcer.check("write", arguments["dst_path"])
                    result = await dsm.file_move(arguments["src_path"], arguments["dst_path"])
                    return [TextContent(type="text", text=json.dumps(result, indent=2))]

                case "syno_file_search":
                    enforcer.check("read", arguments.get("folder_path", "/"))
                    result = await dsm.file_search(arguments["query"], arguments.get("folder_path", "/"))
                    return [TextContent(type="text", text=json.dumps({"results": result, "count": len(result)}, indent=2))]
                ...
                case "syno_storage_info":
                    ...
                    return [TextContent(type="text", text=json.dumps({"volumes": result}, indent=2))]
