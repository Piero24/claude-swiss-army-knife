...
class PathRule(BaseModel):
    ...
    path: str = Field(
        ..., description="Glob pattern for matching paths (e.g., /var/log/**)"
    )
    access: AccessLevel = Field(
        ..., description="Access level granted for this path"
    )
...
class CommandRule(BaseModel):
    ...
    pattern: str = Field(
        ...,
        description="Glob pattern for matching commands (e.g., 'systemctl status *')",
    )
    access: AccessLevel = Field(
        ..., description="Access level required to run this command"
    )
    description: Optional[str] = Field(
        default=None, description="Human-readable description"
    )
...
class PermissionsConfig(BaseModel):
    ...
    default_access: AccessLevel = Field(
        default=AccessLevel.NONE,
        description="Default access when no rule matches",
    )
    paths: list[PathRule] = Field(
        default_factory=list, description="Ordered list of path rules"
    )
    commands: list[CommandRule] = Field(
        default_factory=list, description="Ordered list of command rules"
    )
    default_command_access: AccessLevel = Field(
        default=AccessLevel.NONE,
