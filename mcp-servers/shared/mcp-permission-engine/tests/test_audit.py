...
class TestAuditLogger:
    ...
    def test_user_id_default(self, audit_logger):
        """user_id defaults to 'default' when not provided."""
        audit_logger.allowed("test-mcp", "file", "/test")
        entries = read_audit_log(audit_logger._log_path)
        assert entries[0].get("user_id") == "default"

    def test_user_id_custom_allowed(self, audit_logger):
        """Custom user_id flows through to log entry (allowed)."""
        audit_logger.allowed(
            "test-mcp",
            "file",
            "/test",
    ...
    def test_user_id_custom_denied(self, audit_logger):
        """Custom user_id flows through to log entry (denied)."""
        audit_logger.denied(
            "test-mcp",
            "file",
            "/secret",
